...
function real(){
	...
	if(realDel){
		window.location='deleteBoard.board?num=${dto.num}'
	}
}   
</script>
	...
	<table width="100%">      
		...
		<tr class="tableClass">
			...
			<c:if test="${adminLogin}">
				<input type="button" value="글수정" onclick="window.location='updateForm.board?num=${dto.num}'">
				<input type="button" value="글삭제" onclick="javascript:real()">
     	 	</c:if>
				<input type="button" value="글목록"  onclick="window.location='notice.board'" align="center">
			</td>
		</tr>
	</table>
</div>
